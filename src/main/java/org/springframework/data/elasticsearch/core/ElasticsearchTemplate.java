...
public class ElasticsearchTemplate implements ElasticsearchOperations, ApplicationContextAware {
    ...
    public <T> List<String> queryForIds(SearchQuery query) {
        SearchRequestBuilder request = prepareSearch(query).setQuery(query.getQuery()).storedFields("_none_");
    ...
    public <T> Page<T> queryForPage(StringQuery query, Class<T> clazz, SearchResultMapper mapper) {
        SearchResponse response = getSearchResponse(prepareSearch(query, clazz).setQuery(QueryBuilders.simpleQueryStringQuery(query.getSource())).execute());
        return mapper.mapResults(response, clazz, query.getPageable());
    }
    ...
    private SearchRequestBuilder prepareScan(Query query, long scrollTimeInMillis, boolean noFields) {
        ...
        if (noFields) {
            requestBuilder.storedFields("_none_");
        }
    ...
    public Boolean addAlias(AliasQuery query) {
        ...
        QueryBuilder filterBuilder = query.getFilterBuilder();
        if (filterBuilder != null) {
        }
        AliasAction aliasAction = AliasAction.Add(query.getIndexName(), query.getAliasName(), );
