...
public class ElasticsearchTemplate implements ElasticsearchOperations, ApplicationContextAware {
        ...
        try {
            ...
            if (idProperty != null) {
                xContentBuilder = buildMapping(clazz, persistentEntity.getIndexType(),
                        idProperty.getFieldName(), persistentEntity.getParentType());
            } else {
                throw new IllegalStateException(String.format("Required identifier property not found for %s!", clazz));
            }
    ...
    public ElasticsearchPersistentEntity getPersistentEntityFor(Class clazz) {
        ...
        MappingContext<? extends ElasticsearchPersistentEntity<?>, ElasticsearchPersistentProperty> mappingContext = elasticsearchConverter.getMappingContext();

        ElasticsearchPersistentEntity<?> entity = mappingContext.getPersistentEntity(clazz);
        if (entity != null) {
            return entity;
        }

        throw new MappingException(String.format("Couldn't find PersistentEntity for type %s!", clazz));
    }
